...
func TestMain(m *testing.M) {
	...
	if fullConfig.CredentialStorePath() != "" {
		userStore, err = identitymgr.NewCertFileUserStore(fullConfig.CredentialStorePath())
		if err != nil {
	...
	identityManager, err = identitymgr.New("org1", stateStore, cryptoSuite, fullConfig)
	if err != nil {
...
func TestRegister(t *testing.T) {
	...
	_, err = caManager.Register(&msp.RegistrationRequest{})
	if err == nil {
	...
	secret, err := caManager.Register(&msp.RegistrationRequest{Name: "test", Affiliation: "test", Attributes: attributes})
	if err != nil {
...
func TestEmbeddedRegistar(t *testing.T) {
	...
	identityManager, err := identitymgr.New("org1", stateStore, cryptoSuite, embeddedRegistrarConfig)
	if err != nil {
		t.Fatalf("manager.New returned error: %v", err)
	}
	...
	attributes = append(attributes, msp.Attribute{Key: "test1", Value: "test2"})
	attributes = append(attributes, msp.Attribute{Key: "test2", Value: "test3"})
	secret, err := caManager.Register(&msp.RegistrationRequest{Name: "withEmbeddedRegistrar", Affiliation: "test", Attributes: attributes})
	if err != nil {
		t.Fatalf("identityManager Register return error %v", err)
	}
	if secret != "mockSecretValue" {
...
func TestRegisterNoRegistrar(t *testing.T) {
	...
	identityManager, err := identitymgr.New("org1", stateStore, cryptoSuite, noRegistrarConfig)
	if err != nil {
		t.Fatalf("manager.New returned error: %v", err)
	}
	...
	attributes = append(attributes, msp.Attribute{Key: "test1", Value: "test2"})
	attributes = append(attributes, msp.Attribute{Key: "test2", Value: "test3"})
	_, err = caManager.Register(&msp.RegistrationRequest{Name: "test", Affiliation: "test", Attributes: attributes})
	if err != msp.ErrCARegistrarNotFound {
		t.Fatalf("Expected ErrCARegistrarNotFound, got: %v", err)
	}
}
...
func TestRevoke(t *testing.T) {
	...
	_, err = caManager.Revoke(&msp.RevocationRequest{})
	if err == nil {
...
// TestCreateNewidentityManagerClientCAConfigMissingFailure will test newidentityManager CAClient creation with with CAConfig
...
// TestCreateNewidentityManagerClientCertFilesMissingFailure will test newidentityManager CAClient creation with missing CA Cert files
...
// TestCreateNewidentityManagerClientCertFileErrorFailure will test newidentityManager CAClient creation with missing CA Cert files, additional scenario
...
// TestCreateNewidentityManagerClientKeyFileErrorFailure will test newidentityManager CAClient creation with missing CA Cert files and missing key
...
// TestCreateValidBCCSPOptsForNewFabricClient test newidentityManager CAClient creation with valid inputs, successful scenario
