...
func createChannelAndCC(t *testing.T, sdk *fabsdk.FabricSDK) {
	...
	if metadata.Ccmode == "Lscc" {
		...
		createCCLifecycle(t, orgResMgmt)
	}
...
func createCCLifecycle(t *testing.T, orgResMgmt *resmgmt.Client) {
	// Package cc
	label, ccPkg := packageCC(t, orgResMgmt)
	...
	// Query approve cc
	queryApprovedCC(t, label, packageID, orgResMgmt)
...
func packageCC(t *testing.T, orgResMgmt *resmgmt.Client) (string, []byte) {
...
func queryApprovedCC(t *testing.T, label string, packageID string, orgResMgmt *resmgmt.Client) {
