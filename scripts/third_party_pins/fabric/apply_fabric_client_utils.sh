...
sed -i'' -e '/import (/ a\
    \"github.com\/hyperledger\/fabric-sdk-go\/pkg\/core\/cryptosuite\"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e '/func New(/ a\
...
sed -i'' -e 's/newBccspMsp(MSPv1_4_2)/NewBccspMsp(MSPv1_4_2, cs)/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
